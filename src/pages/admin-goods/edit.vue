  ...
  export default {
    ...
    methods: {
      ...
      async getGoods (goodsId) {
        let {data:{StatusCode, goodInfo}} = await api.get('api/StoreUse/stockManager/OneGoodsDetail.ashx', {goSn: goodsId});
        this.model = Object.assign(this.model, goodInfo[0])
      },
      async getType () {
      ...
      async submit () {
        ...
        this.disabled = true;
        let {data: {StatusCode, Status}} = await api.post('api/StoreUse/stockManager/editGoods.ashx', Object.assign(this.model,{goSn}));
        if(StatusCode == 200) {
