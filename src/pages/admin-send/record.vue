...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.curYear = this.$route.query.year ? parseInt(this.$route.query.year): date.getFullYear();
      this.curMonth = this.$route.query.month ? parseInt(this.$route.query.month): date.getMonth() + 1;
    ...
    methods: {
      ...
      handleCalendar(type) {
        ...
        this.getHistory(this.curYear, this.curMonth);
        this.$router.replace({name:'adminSendRecord', query:{year:this.curYear,month: this.curMonth}})
      },
      getHistory (year, month) {
        this.$store.dispatch('admin/send/getCalendar', {year, month});
      },
      checkGoods (day) {
      ...
      link (day) {
        if(this.checkGoods(day)) {
          ...
          this.$router.push({name: 'adminSend', query: {date: timetamp, record:1}});
        }
      }
    }
  }
