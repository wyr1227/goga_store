...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.curYear = this.$route.query.year ? parseInt(this.$route.query.year): parseInt(date.getFullYear());
      console.log(this.curYear)
      this.curMonth = this.$route.query.month ? parseInt(this.$route.query.month): parseInt(date.getMonth() + 1);
      this.nowDay = date.getDate();
      this.curDay = this.$route.query.day? parseInt(this.$route.query.day): 0
      this.calculateEmptyGrids(this.curYear, this.curYear);
      this.calculateDays(this.curYear,  this.curYear);
    ...
    methods: {
      ...
      handleCalendar(type) {
        ...
        this.getHistory(this.curYear, this.curMonth);
        this.$router.replace({name:'adminJinhuoRecord', query:{year:this.curYear,month: this.curMonth}});
      },
      getHistory (year, month) {
        this.$store.dispatch('admin/jinhuo/getCalendar', {year, month});
      },
      checkGoods (day) {
      ...
      link (day) {
        if(this.checkGoods(day)) {
          ...
          this.$router.push({name: 'adminJinhuo', query: {date: timetamp, record:1}});
        }
      }
    }
  }
